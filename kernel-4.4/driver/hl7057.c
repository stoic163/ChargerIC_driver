...
static struct hl7057_info *g_info;
...
static DEFINE_MUTEX(hl7057_i2c_access);
...
static int hl7057_parse_dt(struct hl7057_info *info, struct device *dev)
{
	...
	if (of_property_read_string(np, "alias_name", &(info->chg_props.alias_name)) < 0) {
		info->chg_props.alias_name = "hl7057";
...
static int hl7057_do_event(struct charger_device *chg_dev, unsigned int event, unsigned int args)
{
	if (chg_dev == NULL)
		return -EINVAL;
...
static int hl7057_enable_charging(struct charger_device *chg_dev, bool en)
{
...
static int hl7057_set_cv_voltage(struct charger_device *chg_dev, u32 cv)
{
	int status = 0;
...
static int hl7057_get_current(struct charger_device *chg_dev, u32 *ichg)
{
	int status = 0;
...
static int hl7057_set_current(struct charger_device *chg_dev, u32 current_value)
{
	unsigned int status = 0;
...
static int hl7057_get_input_current(struct charger_device *chg_dev, u32 *aicr)
{
	unsigned int status = 0;
	unsigned int array_size;
	unsigned int register_value;
...
static int hl7057_set_input_current(struct charger_device *chg_dev, u32 current_value)
{
	unsigned int status = 0;
	unsigned int set_chr_current;
	unsigned int array_size;
...
static int hl7057_get_charging_status(struct charger_device *chg_dev, bool *is_done)
{
	unsigned int status = 0;
...
static int hl7057_reset_watch_dog_timer(struct charger_device *chg_dev)
{
	hl7057_set_tmr_rst(1);
	return 0;
}

static int hl7057_charger_enable_otg(struct charger_device *chg_dev, bool en)
{
	if (en) {
		hl7057_set_otg_en(0x1);
		hl7057_set_hz_mode(0x0);
		hl7057_set_opa_mode(0x1);
		hl7057_set_otg_pl(0x1);
...
static void usbotg_boost_kick_work(struct work_struct *work)
{
	...
	struct hl7057_info *boost_manager =
		container_of(work, struct hl7057_info, kick_work);
	...
	if (boost_manager->polling_enabled == true)
		gtimer_start(&boost_manager->otg_kthread_gtimer, boost_manager->polling_interval);
}

static int usbotg_gtimer_func(struct gtimer *data)
{
	struct hl7057_info *boost_manager =
		container_of(data, struct hl7057_info, otg_kthread_gtimer);
...
static struct charger_ops hl7057_chg_ops = {
	...
	.enable = hl7057_enable_charging,
	.get_charging_current = hl7057_get_current,
	.set_charging_current = hl7057_set_current,
	.get_input_current = hl7057_get_input_current,
	.set_input_current = hl7057_set_input_current,
	/*.get_constant_voltage = hl7057_get_battery_voreg,*/
	.set_constant_voltage = hl7057_set_cv_voltage,
	.kick_wdt = hl7057_reset_watch_dog_timer,
	.is_charging_done = hl7057_get_charging_status,
	/* OTG */
	.enable_otg = hl7057_charger_enable_otg,
	.event = hl7057_do_event,
};

static int hl7057_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret = 0;
	struct hl7057_info *info = NULL;

	pr_info("[hl7057_driver_probe]\n");
	info = devm_kzalloc(&client->dev, sizeof(struct hl7057_info), GFP_KERNEL);
	...
	new_client = client;
	info->dev = &client->dev;
	ret = hl7057_parse_dt(info, &client->dev);
	...
	info->chg_dev = charger_device_register(info->chg_dev_name,
		&client->dev, info, &hl7057_chg_ops, &info->chg_props);
	...
	/* hl7057_hw_init(); //move to charging_hw_xxx.c */
...
static const struct of_device_id hl7057_of_match[] = {
	...
	{.compatible = "mediatek,swithing_charging"},
	{},
};
#else
static struct i2c_board_info i2c_hl7057 __initdata = {
	I2C_BOARD_INFO("hl7057", (hl7057_SLAVE_ADDR_WRITE >> 1))
};
#endif
...
static int __init hl7057_init(void)
{
	...
	if (i2c_add_driver(&hl7057_driver) != 0)
		pr_info("Failed to register hl7057 i2c driver.\n");
	else
		pr_info("Success to register hl7057 i2c driver.\n");
